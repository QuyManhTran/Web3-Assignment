...
import { JwtAccessTokenProviderOptions, JwtKey } from '#types/auth'
...
export class JwtKeyPair implements JwtKey {
    ...
    constructor(publicKey: string, privateKey: string, refreshTokenKey: string) {
        this.verificationKey = publicKey
        this.secret = privateKey
        this.refreshSecret = refreshTokenKey
    }
}
...
export class JwtSecret implements JwtKey {
    ...
    readonly secret: string
    readonly refreshSecret: string
    constructor(secret: string, refreshTokenKey: string) {
    ...
    implements AccessTokensProviderContract<TokenableModel>
{
    ...
    async createRefresh(
        ...
        const expiresInMillis = options?.expiresIn ?? this.options.refreshExpiresInMillis
        ...
        const jwtToken = jwt.sign(
            ...
            this.options.key.refreshSecret,
            {
                algorithm: this.algorithm,
                issuer: this.options.issuer,
                audience: this.options.audience,
